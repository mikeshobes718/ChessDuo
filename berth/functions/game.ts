...
// Berth injects these into every function; the secret key never leaves the server.
const berthApiUrl = Deno.env.get("BERTH_API_URL") ?? "";
const berthApp = Deno.env.get("BERTH_APP") ?? "";
const berthKey = Deno.env.get("BERTH_SECRET_KEY") ?? "";
...
// Berth stops a function as soon as it returns, so background work (pushes, the AI
// coach, presence) is collected per request and awaited before the response goes out.
const backgroundTasks = new AsyncLocalStorage<Promise<unknown>[]>();
const BACKGROUND_BUDGET_MS = 12000;

function waitUntil(task: Promise<unknown>) {
  const tasks = backgroundTasks.getStore();
  const guarded = task.catch((error) => console.error("background task failed", error));
  if (tasks) tasks.push(guarded);
}

async function drainBackground(tasks: Promise<unknown>[]) {
  if (!tasks.length) return;
  // Tasks can queue more tasks (the coach writes back to the game), so drain until quiet.
  const deadline = new Promise((resolve) => setTimeout(resolve, BACKGROUND_BUDGET_MS));
  const drain = (async () => {
    let seen = 0;
...
// Takes the PostgREST-style path the game logic was written against ("games?id=eq.x")
// and sends it to the Berth rows API, returning the row array like PostgREST did.
async function database(path: string, init: RequestInit = {}) {
  const [table, rawQuery = ""] = path.split("?");
  const query = new URLSearchParams(rawQuery);
  if (query.get("select") === "*") query.delete("select");
  if (query.has("on_conflict")) query.set("upsert", "true");
  const search = query.toString();
  const headers = new Headers(init.headers ?? {});
  headers.delete("Prefer");
  headers.set("Authorization", `Bearer ${berthKey}`);
  const response = await fetch(
    `${berthApiUrl}/apps/${berthApp}/tables/${table}/rows${search ? `?${search}` : ""}`,
    { ...init, headers },
  );
  if (!response.ok) {
    const detail = await response.text();
...
// Berth has no or= filter, so "white or black player" is two queries merged.
async function archivesForPlayer(hash: string, params: Record<string, string>, limit: number) {
  const lists = await Promise.all(["white_token_hash", "black_token_hash"].map((column) =>
    database(`match_archives?${new URLSearchParams({ ...params, [column]: `eq.${hash}`, order: "ended_at.desc", limit: String(limit) })}`)
  ));
  const byId = new Map<string, Record<string, unknown>>();
  for (const row of lists.flat()) byId.set(String(row.id), row);
  return [...byId.values()]
...
async function listArchives(body: Record<string, unknown>) {
  ...
  if (!token) throw new HttpError("Missing player token", 401);
  const hash = await hashToken(token);
  const rows = await archivesForPlayer(hash, {
...
async function getArchive(body: Record<string, unknown>) {
  ...
  const hash = await hashToken(token);
  const rows = await archivesForPlayer(hash, { id: `eq.${id}` }, 1);
  const row = rows?.[0];
...
Deno.serve((request) => {
  ...
  return backgroundTasks.run(tasks, async () => {
    const response = await handle(request);
    await drainBackground(tasks);
    return response;
  });
});
...
async function handle(request: Request) {
  ...
  if (!berthApiUrl || !berthApp || !berthKey) return json({ error: "Server configuration is incomplete" }, 500);
  ...
  try {
    ...
    const actions: Record<string, (value: Record<string, unknown>) => Promise<unknown>> = {
      ...
      state: async (value) => {
        ...
        waitUntil(touchPresence(game, color));
